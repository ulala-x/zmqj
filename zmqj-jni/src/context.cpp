 ...
 */
JNIEXPORT void JNICALL Java_org_zeromq_Context__1construct(JNIEnv* env, jobject obj, jint io_threads)
{
 ...
 */
JNIEXPORT void JNICALL Java_org_zeromq_Socket_destroy(JNIEnv *env, jobject obj)
{
    void *c = get_context (env, obj);
    if (! c)
        return;
...
JNIEXPORT jboolean JNICALL Java_org_zeromq_Context__1setMaxSockets(JNIEnv * env, jobject obj, jint maxSockets)
{
    void *c = get_context (env, obj);
    if (! c)
        return JNI_FALSE;
...
JNIEXPORT jint JNICALL Java_org_zeromq_Context__1getMaxSockets(JNIEnv * env, jobject obj)
{
    void *c = get_context (env, obj);
    if (! c)
        return -1;
